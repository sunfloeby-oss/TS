  ...
  try {
    ...
    const response = await fetch(url, {
      headers: {
        ...
        "user-agent": "terrashop20-storefront (shipping rate lookup)",
      },
    });
 ...
 *
 * TODO: this still holds the previous brand's pickup address as a
 * placeholder. Replace every line below with Terrashop20's actual
 * warehouse/pickup address, then re-run `resolveStoreAreaId()` once and
 * paste the resulting area ID into `BITESHIP_ORIGIN_AREA_ID` — shipping
 * rates will be quoted from whatever address is here until that's done.
 */
const STORE_ADDRESS = [
  "REPLACE_WITH_TERRASHOP20_STREET_ADDRESS",
  "REPLACE_WITH_KELURAHAN",
  "REPLACE_WITH_KECAMATAN",
  "REPLACE_WITH_CITY",
  "REPLACE_WITH_POSTAL_CODE",
].join(", ");
